...
void Gimbal::rotateTo(double phi, double theta, int angleSpeed_phi,int angleSpeed_theta){ //horizontal axis in deg, vertical axis in deg, anglespeed in deg/sec | phi -> stepperOne, theta -> stepperTwo
  ...
  while (abs(Dphi) > 0.1 || abs(Dtheta) > 0.1) {
    if (Dphi != 0) {
      ...
      phi_dot = (angleSpeed_phi) / (sqrt(pow(cos(_stepperOne->getAbsoluteAngle() * (PI / 180.0)), 2) * pow(c, 2) + 1));
    ...
    if (Dtheta != 0) {
      ...
      if (Dphi != 0) {
        c = abs(Dtheta) / abs(Dphi);
        theta_dot = (c * angleSpeed_theta) / (sqrt(pow(cos(_stepperOne->getAbsoluteAngle() * (PI / 180.0)), 2) * pow(c, 2) + 1));
        theta_interval = anglTouSec(theta_dot);
      } else {
        if (_stepperOne->getAbsoluteAngle() == 90) {
          theta_interval = _stepperTwo->getMinInterval();
        } else {
          theta_dot = (angleSpeed_theta) / (sqrt(pow(cos(_stepperOne->getAbsoluteAngle() * (PI / 180.0)), 2))); //90° abfangen
          theta_interval = anglTouSec(theta_dot);
        }
      }
